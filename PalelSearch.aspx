<%@ Page Language="C#" AutoEventWireup="true" CodeFile="PalelSearch.aspx.cs" Inherits="PalelSearch" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title></title>
    <style type="text/css">
        .auto-style2 {
            width: 193px;
        }
        .auto-style3 {
            width: 146px;
        }
        .auto-style4 {
            width: 283px;
        }
    </style>
</head>
<body>
    <form id="form1" runat="server">
    <div>
    <center>
        Enter EmployeeID: <asp:TextBox ID="TextBox1" runat="server" Height="22px" Width="267px"></asp:TextBox>
        <asp:Button ID="Button1" runat="server" BorderStyle="Inset" Font-Bold="True" ForeColor="#000066" Height="30px" Text="Search" Width="80px" OnClick="Button1_Click" />
        <asp:Label ID="Lblmsg" runat="server"></asp:Label>
    </center>
    </div>
        <div>
            <asp:Panel ID="Panel1" runat="server" BorderStyle="Inset">
                <table style="width:100%;">
                    <tr>
                        <td class="auto-style4">&nbsp;</td>
                        <td class="auto-style3">Name:</td>
                        <td class="auto-style2">
                            <asp:Label ID="Lblnm" runat="server"></asp:Label>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="auto-style4">&nbsp;</td>
                        <td class="auto-style3">Address:</td>
                        <td class="auto-style2">
                            <asp:Label ID="Lbladr" runat="server"></asp:Label>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="auto-style4">&nbsp;</td>
                        <td class="auto-style3">Contact Number:</td>
                        <td class="auto-style2">
                            <asp:Label ID="Lblph" runat="server"></asp:Label>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="auto-style4">&nbsp;</td>
                        <td class="auto-style3">Salary:</td>
                        <td class="auto-style2">
                            <asp:Label ID="Lblsal" runat="server"></asp:Label>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="auto-style4">&nbsp;</td>
                        <td class="auto-style3">City:</td>
                        <td class="auto-style2">
                            <asp:Label ID="Lblcity" runat="server"></asp:Label>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="auto-style4">&nbsp;</td>
                        <td class="auto-style3">Gender:</td>
                        <td class="auto-style2">
                            <asp:Label ID="Lblgen" runat="server"></asp:Label>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="auto-style4">&nbsp;</td>
                        <td class="auto-style3">Department:</td>
                        <td class="auto-style2">
                            <asp:Label ID="Lbldep" runat="server"></asp:Label>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                    <tr>
                        <td class="auto-style4">&nbsp;</td>
                        <td class="auto-style3">Company:</td>
                        <td class="auto-style2">
                            <asp:Label ID="Lblcmp" runat="server"></asp:Label>
                        </td>
                        <td>&nbsp;</td>
                    </tr>
                </table>
            </asp:Panel>
        </div>
    </form>
</body>
</html>
